...
import {startExperience,finishExperience,experienceTable,attachSqueeze,revealSqueeze,squeezeOrder,renderCards,renderBlackjackCards} from './experience.js';
...
function cards(cs,hidden=false){return (s.game==='blackjack'?renderBlackjackCards:renderCards)(cs,{hidden:i=>hidden&&i>0});}
...
if(!a&&r&&s.lastHand?.kind===s.game&&s.game==='poker'){const h=s.lastHand;return `<div class="caption">${s.game==='poker'?'CPU · SHOWDOWN':'DEALER'}</div>${s.game==='poker'?renderCards(h.cpu,{hidden:()=>!h.showdown}):cards(h.d)}${s.game==='poker'&&h.board.length?cards(h.board):''}<div class="caption">YOUR HAND</div>${cards(h.p)}<div class="message" role="status">${r.message}<br><strong class="${r.delta>=0?'positive':'negative'}">${r.delta>=0?'+':''}${money(r.delta)} COINS</strong></div><div class="actions">${button('start','次のハンドへ',s.balance<s.stake,true)}</div>`;}
...
if(s.game==='blackjack'){
 const h=a||(r&&s.lastHand?.kind==='blackjack'?s.lastHand:null);
 if(h){html=`<div class="experience-heading"><span class="eyebrow">BLACKJACK · LIVE TABLE</span><h2>${a?'あなたのターン。':'勝負の結果。'}</h2></div><div class="caption">DEALER</div>${cards(h.d,!!a)}<div class="table-divider"><span>BLACKJACK PAYS 3 TO 2</span></div><div class="caption">YOUR HAND</div>${cards(h.p)}${a?`<div class="experience-status">あなた ${bj(h.p)} · BET ${money(h.bet)}</div><div class="actions">${button('hit','ヒット',false,true)}${button('stand','スタンド')}${button('double','ダブル',h.p.length!==2||s.balance<h.bet)}</div>`:`<div class="actions">${button('start','次のハンドへ',s.balance<s.stake,true)}</div>`}`;
 }else html=`<div class="caption">BLACKJACK · DEALER STANDS ON 17</div><h2>21へ、あと一歩。</h2>${cards([{r:14,s:0},{r:13,s:1}])}<p>引くか、止まるか。判断を学ぼう。</p>${button('start','カードを配る',s.balance<s.stake,true)}`;
}
