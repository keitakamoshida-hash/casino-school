...
function wheel(s) {
  ...
  return `<div class="roulette-wheel ${spin?'is-spinning':''}" style="${motion}" aria-label="前回の出目 ${s.lastNumber??'未実施'}"><span class="wheel-pointer"></span><div class="ball-track" aria-hidden="true"></div><div class="ball-orbit" aria-hidden="true"><span class="roulette-ball"></span></div><svg viewBox="0 0 300 300" aria-hidden="true" style="transform:rotate(${rotation}deg)">${sectors}<circle cx="150" cy="150" r="94" fill="#70492d" stroke="#d8b873" stroke-width="4"/><circle cx="150" cy="150" r="78" fill="#223a2b" stroke="#aa8b54" stroke-width="2"/></svg><div class="wheel-center"><small>${spin?'SPINNING':'LAST NUMBER'}</small><strong class="${s.lastNumber===0?'zero-text':red.has(s.lastNumber)?'red-text':''}">${spin?'?':s.lastNumber??'—'}</strong><span>${spin?'NO MORE BETS':'EUROPEAN'}</span></div></div>`;
}
export function rouletteTable(s) {
  ...
  return `<div class="roulette-title"><div><span class="eyebrow">THE ROULETTE ROOM</span><h2>チップを置いて、運を試す。</h2></div><span class="pill">SINGLE ZERO · 0–36</span></div>
    ...
    <div class="roulette-arena ${busy?'spin-focus':''}"><div class="wheel-panel">${wheel(s)}<div class="recent-numbers" aria-label="過去の出目">${s.rouletteRecent.slice(0,7).map(n=>`<span class="${n===0?'zero':red.has(n)?'number-red':'number-black'}">${n}</span>`).join('')||'<small>最初のスピンを待っています</small>'}</div><p class="wheel-note">37の数字。どの数字も同じ確率。</p></div>
